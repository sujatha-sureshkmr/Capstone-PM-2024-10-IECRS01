...
def extract_text_with_style(pdf_file):
    df_data = []
    ...
    data_df_prep = dataprep(df)
    data_df = buildhierarchy(data_df_prep)
    return data_df

def order_numerical_column_desc_start_one(df, column_name):
...
def extract_columns(df_expanded_span, df1_columns):
    data_dict = {}
...
def save_output(data, output_folder, filename):
...
def dataprep(data_df):
    ...
    df_expanded = expand_list_column(data_df, 'Bbox', new_column_names)
    ...
    df1 = extract_columns(df_expanded, df1_columns)
    df1 = df1[df_expanded['Text'].notna()]
    df1['image_name'] = np.NaN
    df2 = extract_columns(df_expanded, df2_columns)
    ...
    save_output(df2,"./knowledge_graph_files",'knowledge_graph_image.csv')
    ...
    filtered_indices = df1[df1.apply(lambda row: (pd.isna(row['image_name'])) and len(str(row['Text']).split()) == 1 and is_not_proper_word(str(row['Text'])), axis=1)].index
    ...
    data_df['Header_style'] = order_numerical_column_desc_start_one(data_df, 'font_size_diff')
    return data_df
